...
def main(mission_dir, ncfile_name, start_time_str, end_time_str):
    '''
    @edwinrainville
    ...
    # Open netCDF dataset
    ncfile_loc = mission_dir + ncfile_name
    rootgrp = nc.Dataset(ncfile_loc, 'w', clobber=True)
    ...
    microSWIFT_dir_list = glob.glob(microSWIFT_dir + 'microSWIFT_*')
...
if __name__=='__main__':
    mission_dir = str(input('Mission Directory: '))
    ncfile_name = str(input('Name of Output NC file: '))
    start_time = str(input('Start time(YYYY-MM-DDTHH:MM:SS UTC): '))
    end_time = str(input('End time(YYYY-MM-DDTHH:MM:SS UTC): '))
    main()
